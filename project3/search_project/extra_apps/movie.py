...
def douban_movie_info():
	...
	for movie_info in movie_info_list:
	    ...
	    path = "./image/image"+str(i)
	    dowloadPic(movie_image_url,path)
	    movie_dict = dict(movie_time=movie_time, movie_id=movie_id, 
...
def movie_subject():
	...
	movie_info = soup.find("div",{"id":"info"}).text
...
def douban_movie_info():
	url = 'https://movie.douban.com/j/cinema/schedules?date=2017-10-02&s_id=25723583&site_id=274933'
	response = requests.get(url)
	json_html = json.loads(response.text)
	movie_info_list = []
